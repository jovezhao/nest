...
为实体定义仓储实现，Repository接口是一个泛型接口，根据要求需要输入一个`Entity`的实现类。
...
```
查询接口的定义可以在应用服务层，也可以在领域层。取决后该查询接口的使用对象。如果定义在领域层，建议只返回实体的ID列表。如果定义在应用服务层，可以直接返回DTO对象，请勿直接返回领域对象，避免破坏或泄露领域对象的业务细节。
